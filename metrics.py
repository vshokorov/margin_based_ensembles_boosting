...
def compute_test_metrics(preds, targets,
                         disable=('misclass_MI_auroc', 'sce', 'ace', 'misclass_entropy_auroc@5', 'misclass_confidence_auroc@5'), **args):
    metric_name_to_f = {
        'acc': get_acc,
        'acc@5': get_acc5,
        'll': get_ll,
    ...
    for metric, f in metric_name_to_f.items():
        if metric not in disable:
            # start = time.time()
            res[metric] = f(preds, targets, **args)
            # s += time.time() - start
            # print(metric, time.time() - start)
    # print('sum', s)
    # print()
    return res
...
def metrics_kfold(
        log_preds, targets, n_splits=2, n_runs=5, verbose=False, temp_scale=False,
        disable=('misclass_MI_auroc', 'sce', 'ace', 'misclass_entropy_auroc@5', 'misclass_confidence_auroc@5'), **args):
    ...
    for runs in range(n_runs):
        for i, (tr_idx, te_idx) in enumerate(KFold(n_splits=n_splits, shuffle=True).split(log_preds)):
            ...
            te_metrics = compute_test_metrics(np.exp(test_lp), targets[te_idx], disable=disable, **args)
