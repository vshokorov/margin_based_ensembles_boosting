...
class WideBasic(nn.Module):
    def __init__(self, in_planes, planes, p, stride=1):
...
class WideBasicCurve(nn.Module):
    def __init__(self, in_planes, planes, p, fix_points, stride=1):
...
class WideResNetBase(nn.Module):
    def __init__(self, num_classes, depth=28, k=160, p=0., **kwargs):
        ...
        self.layer1 = self._wide_layer(WideBasic, nstages[1], n, p, stride=1)
        self.layer2 = self._wide_layer(WideBasic, nstages[2], n, p, stride=2)
        self.layer3 = self._wide_layer(WideBasic, nstages[3], n, p, stride=2)
...
class WideResNetCurve(nn.Module):
    def __init__(self, num_classes, fix_points, depth=28, k=10, p=0):
        ...
        self.layer1 = self._wide_layer(WideBasicCurve, nstages[1], n, p, stride=1,
                                       fix_points=fix_points)
        self.layer2 = self._wide_layer(WideBasicCurve, nstages[2], n, p, stride=2,
                                       fix_points=fix_points)
        self.layer3 = self._wide_layer(WideBasicCurve, nstages[3], n, p, stride=2,
                                       fix_points=fix_points)
    ...
    def _wide_layer(self, block, planes, num_blocks, p, stride, fix_points):
        strides = [stride] + [1] * int(num_blocks - 1)
        layers = []

        for stride in strides:
            layers.append(
                block(self.in_planes, planes, p, fix_points=fix_points, stride=stride)
            )
            self.in_planes = planes
...
class WideResNet28x10:
    def __init__(self):
        self.base = WideResNetBase
        self.curve = WideResNetCurve
        self.kwargs = {'depth': 28, 'k': 160}
