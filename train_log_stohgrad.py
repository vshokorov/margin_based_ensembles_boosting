    ...
    for exp_num in range(args.num_exps):
        ...
        while num_model < args.num_nets:
            ...
            for epoch in range(start_epoch, args.epochs + 1):
                ...
                train_res = utils.train_with_grad_log(loaders['train'], model, optimizer, criterion, device, regularizer)
                ...
                grad_norm.append(np.array(train_res['grad_norm']))
                opt_step_norm.append(np.array(train_res['opt_step_norm']))
            ...
            np.save(log.path+'/grad_norm_per_step%d.npy' % num_model,
                    np.vstack(grad_norm))
            np.save(log.path+'/opt_step_norm_per_step%d.npy' % num_model,
                    np.vstack(opt_step_norm))
